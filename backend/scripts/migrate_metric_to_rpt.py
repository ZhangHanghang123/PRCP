...
with engine.begin() as conn:
    ...
    # 改唯一键为 (report_id, code, name)，因为 RWA 同 cat 内 code 重复
    try:
        conn.execute(text("ALTER TABLE prcp_rpt_item DROP INDEX uk_report_code"))
        print("✅ DROP old unique key")
    except Exception as e:
        print(f"⚠️  DROP unique error: {e}")
    try:
        conn.execute(text("ALTER TABLE prcp_rpt_item ADD UNIQUE KEY uk_report_code_name (report_id, item_code, item_name)"))
        print("✅ ADD new unique key (report_id, item_code, item_name)")
    except Exception as e:
        print(f"⚠️  ADD unique error: {e}")
...
with engine.begin() as conn:
    # 先清空 prcp_rpt_item（之前 verify 测试残留）
    n_old = conn.execute(text("UPDATE prcp_rpt_item SET is_deleted=1")).rowcount
    print(f"  软删旧 prcp_rpt_item: {n_old} 条")
