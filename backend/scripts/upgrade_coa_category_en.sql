...
-- 2. 同步 COA 节点 category 字段
UPDATE prcp_coa_node SET category='ASSET'       WHERE category='资产';
UPDATE prcp_coa_node SET category='LIABILITY'   WHERE category='负债';
UPDATE prcp_coa_node SET category='EQUITY'      WHERE category='权益';
UPDATE prcp_coa_node SET category='OFF_BALANCE' WHERE category='表外';
...
-- 5. 同步 ZXCOA 路径（path 中 L1_资产 / L1_负债 / L1_权益 / L1_表外 → L1_ASSET 等）
-- 注意：path 是层次编码，影响前端过滤和后端聚合
...
SELECT category, COUNT(*) cnt FROM prcp_coa_node GROUP BY category ORDER BY category;
